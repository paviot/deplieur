...
namespace cpn_ddd
{
  ...
  variable_order_same_t::variable_order_same_t (const variable_order_t::infos_t variables) :
    variable_order_t(variables)
  {
    this->reorder();
  }

  variable_order_same_t::variable_order_same_t (const Place& place) :
    variable_order_t(place)
  {
    this->reorder();
  }

  variable_order_same_t::variable_order_same_t (const Transition& transition) :
    variable_order_t(transition)
  {
    this->reorder();
  }

  variable_order_same_t::~variable_order_same_t ()
  {}

  variable_order_t::conversion_t
  variable_order_same_t::set_order (const variable_order_t::infos_t variables)
  {
    conversion_t result;
    position_t pos = 0;
