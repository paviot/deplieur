...
#ifndef __CPN_DDD__VARIABLE_ORDER_FULL__
#define __CPN_DDD__VARIABLE_ORDER_FULL__
...
namespace cpn_ddd
{
   ...
   */
  class variable_order_full_t : public variable_order_t
  {
  protected:
    conversion_t set_order (const infos_t variables);
    static position_t next_position_;
  public:
    variable_order_full_t (const infos_t variables);
    variable_order_full_t (const Transition& transition);
    variable_order_full_t (const Place& place);
    ~variable_order_full_t ();
  };

}

#endif //__CPN_DDD__VARIABLE_ORDER_FULL__
