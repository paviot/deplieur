...
#ifndef __CPN_DDD__VARIABLE_ORDER_REVERSE__
#define __CPN_DDD__VARIABLE_ORDER_REVERSE__
...
namespace cpn_ddd
{
   ...
   */
  class variable_order_reverse_t : public variable_order_t
  {
  protected:
    conversion_t set_order (const infos_t variables);
  public:
    variable_order_reverse_t (const infos_t variables);
    variable_order_reverse_t (const Transition& transition);
    variable_order_reverse_t (const Place& place);
    ~variable_order_reverse_t ();
  };

}

#endif //__CPN_DDD__VARIABLE_ORDER_REVERSE__
